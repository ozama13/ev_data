...
# Execute Queries and Store Results in CSV Files
for query_name, query in queries.items():
    try:
        ...
        with engine.connect() as conn:
            ...
            # If the result is a query that returns rows (like SELECT), fetch and store the results
            if result.returns_rows:
                ...
                # Save to CSV file
                file_name = f"{query_name.replace(' ', '_').replace('(', '').replace(')', '')}.csv"
